...
import {
  ...
  Info, Trash2, XCircle, Check, RotateCcw, Settings2, Megaphone, MessageSquare, Moon, Sun
} from 'lucide-react'
...
interface DeptInfo {
  code: string; short: string; full: string
}
const DEPARTMENTS = {
  GMIM: {
    code: 'GMIM', short: 'GMİM', full: 'Gemi Makineleri İşletme Mühendisliği',
  },
  DUIM: {
    code: 'DUIM', short: 'DUİM', full: 'Deniz Ulaştırma İşletme Mühendisliği',
  },
} as const satisfies Record<string, DeptInfo>
...
type CurrentUser = Pick<ResearchAssistant, 'id' | 'name'> & Pick<SessionUser, 'role' | 'department'>
...
type PortalView = 'dashboard' | 'announcements' | 'approvals' | 'tasks' | 'exams' | 'schedule' | 'import' | 'categories' | 'personnel'
...
export default function Home({
  ...
  // Auth authority is the verified server session; localStorage is never trusted as identity.
  ...
  const [selectedDept, setSelectedDept] = useState<string | null>(() => {
    if (typeof window === 'undefined') return null
    try {
      const user = portalSession || readStoredUser()
      if (user) {
  ...
  useEffect(() => {
    ...
    setCurrentUser(portalSession)
    const storedDept = normalizeDepartment(localStorage.getItem('gmim_selected_dept'))
    const effectiveDept = portalSession.role === 'dekan' ? (storedDept || portalSession.department) : portalSession.department
    setSelectedDept(effectiveDept)
  }, [portalSession])

  useEffect(() => {
    if (pathname !== '/' || !currentUser) return
    router.replace(currentUser.role === 'user' ? '/tasks' : '/dashboard')
  ...
  useEffect(() => {
    if (openPeriodDialog && currentUser) setShowResetDialog(true)
  }, [currentUser, openPeriodDialog])
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.replace(role === 'user' ? '/tasks' : '/dashboard')
        fetchData()
  ...
  if (loading) {
    return <div className="min-h-dvh flex items-center justify-center bg-background">
      <div className="text-center space-y-4">
        <div className="animate-spin h-12 w-12 border-4 border-brand-navy border-t-brand-gold rounded-full mx-auto"></div>
        <p className="text-text-secondary font-medium">Portal yükleniyor...</p>
      </div>
    </div>
  }
      ...
      <div className="min-h-dvh flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-2xl">
          <div className="text-center mb-10">
            <div className="h-16 w-16 rounded-xl bg-brand-navy flex items-center justify-center mx-auto mb-4">
              <Ship className="h-8 w-8 text-white" />
            </div>
            <h1 className="text-2xl font-bold text-text-primary">İTÜ Denizcilik Fakültesi</h1>
            <p className="text-sm text-text-secondary mt-1">Ar.Gör Yönetim Sistemi · Bölüm seçin</p>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <button data-department="GMIM" onClick={() => chooseDept('GMIM')} className="group min-h-44 text-left rounded-xl border border-department/35 bg-surface p-6 hover:border-department transition-colors">
              <div className="h-12 w-12 rounded-xl bg-department flex items-center justify-center mb-4"><Ship className="h-6 w-6 text-white" /></div>
              <h2 className="text-xl font-bold text-text-primary">GMİM</h2>
              <p className="text-sm text-text-secondary mt-1">Gemi Makineleri İşletme Mühendisliği</p>
              <span className="inline-flex items-center gap-1 text-department text-sm font-semibold mt-4">Giriş yap <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span>
            </button>
            <button data-department="DUIM" onClick={() => chooseDept('DUIM')} className="group min-h-44 text-left rounded-xl border border-department/35 bg-surface p-6 hover:border-department transition-colors">
              <div className="h-12 w-12 rounded-xl bg-department flex items-center justify-center mb-4"><Ship className="h-6 w-6 text-white" /></div>
              <h2 className="text-xl font-bold text-text-primary">DUİM</h2>
              <p className="text-sm text-text-secondary mt-1">Deniz Ulaştırma İşletme Mühendisliği</p>
              <span className="inline-flex items-center gap-1 text-department text-sm font-semibold mt-4">Giriş yap <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span>
            </button>
          </div>
          <p className="text-center text-xs text-text-secondary mt-8">İTÜ DF Ar.Gör Yönetim Sistemi · AI Destekli v3.1</p>
        </div>
      </div>
    )
  }
  ...
  if (!currentUser) {
    return (
      <div data-department={viewDept} className="min-h-dvh flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="h-16 w-16 rounded-xl bg-department flex items-center justify-center mx-auto mb-4">
              <Ship className="h-8 w-8 text-white" />
            </div>
            <h1 className="text-2xl font-bold text-text-primary">{deptInfo?.short} Ar.Gör Yönetim</h1>
            <p className="text-sm text-text-secondary mt-1">İTÜ Denizcilik Fakültesi · {deptInfo?.full}</p>
          </div>
          <Card className="border-border shadow-none">
            <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-lg"><LogIn className="h-5 w-5 text-department" /> Sisteme Giriş</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleLogin} className="w-full min-h-11 bg-department text-department-foreground hover:bg-department/90 gap-2"><LogIn className="h-4 w-4" /> Giriş Yap</Button>
              <Button variant="ghost" onClick={() => { setSelectedDept(null); try { localStorage.removeItem('gmim_selected_dept') } catch {} }} className="w-full min-h-11 text-text-secondary gap-2 text-xs"><RotateCcw className="h-3.5 w-3.5" /> Bölüm değiştir</Button>
            </CardContent>
          </Card>
          <p className="text-center text-xs text-text-secondary mt-6">{deptInfo?.short} Ar.Gör Yönetim Sistemi · AI Destekli v3.1</p>
        </div>
      </div>
    )
  }
  ...
  return (
        ...
        <>
            {isDekan && (
              <Select value={viewDept || 'GMIM'} onValueChange={(v) => { setSelectedDept(v); try { localStorage.setItem('gmim_selected_dept', v) } catch {} }}>
                <SelectTrigger aria-label="Görüntülenen bölüm" className="h-11 w-[96px] text-xs flex-shrink-0"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <Dialog open={showNotifDialog} onOpenChange={setShowNotifDialog}>
              <DialogTrigger asChild>
                <Button variant="ghost" size="icon" className="relative h-11 w-11" aria-label="Bildirimler">
            ...
            {currentUser ? (
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
                  <DialogTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-11 w-11" title="Şifre Değiştir" aria-label="Şifre değiştir"><Settings2 className="h-4 w-4" /></Button>
                  </DialogTrigger>
                ...
                <Button variant="ghost" size="icon" className="h-11 w-11" aria-label="Çıkış yap" onClick={handleLogout}><LogOut className="h-4 w-4" /></Button>
              </div>
            ) : (
    ...
    >
        <Tabs value={activeTab} className="mx-auto max-w-7xl space-y-6">
          {(activeTab === 'exams' || activeTab === 'schedule') && (
            <nav aria-label="Takvim görünümleri" className="flex w-fit rounded-lg border border-border bg-surface-muted p-1">
              <Link
                href="/calendar"
                aria-current={activeTab === 'exams' ? 'page' : undefined}
                className={`flex min-h-11 items-center rounded-md px-4 text-sm font-medium ${activeTab === 'exams' ? 'bg-surface text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
              >
                Sınavlar
              </Link>
              <Link
                ...
                className={`flex min-h-11 items-center rounded-md px-4 text-sm font-medium ${activeTab === 'schedule' ? 'bg-surface text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
              >
                Haftalık program
              </Link>
            </nav>
