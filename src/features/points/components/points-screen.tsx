  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="text-sm font-medium text-primary">{department} çalışma alanı</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Puan Tablosu</h1><p className="mt-1 text-sm text-text-secondary">Bu bir başarı sıralaması değildir. Düşük puan, yeni görev dağılımında daha yüksek öncelik anlamına gelir.</p></div>
      ...
      <section aria-labelledby="active-points-title" className="overflow-hidden rounded-xl border border-border bg-surface">
        <div className="flex flex-col gap-1 border-b border-border px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div className="flex items-center gap-2"><TableProperties aria-hidden="true" className="size-4 text-primary" /><h2 id="active-points-title" className="font-semibold">Görevlendirme önceliği</h2></div><span className="text-xs text-text-secondary">1. kişi yeni görev için en önceliklidir</span></div>
        {activePeople.length ? <ol className="divide-y divide-border">{activePeople.map((person, index) => <PointRow key={person.id} person={person} rank={index + 1} totalRanked={activePeople.length} canViewDetails={data.canViewDetails} expanded={expandedId === person.id} onToggle={() => setExpandedId(expandedId === person.id ? null : person.id)} />)}</ol> : <p className="px-5 py-10 text-center text-sm text-text-secondary">Aktif araştırma görevlisi bulunmuyor.</p>}
      </section>
...
function PointRow({ person, rank, totalRanked, canViewDetails, expanded, onToggle }: { person: PointPerson; rank?: number; totalRanked?: number; canViewDetails: boolean; expanded: boolean; onToggle: () => void }) {
  const content = <><span aria-label={rank && totalRanked ? `Görevlendirme önceliği ${rank} / ${totalRanked}` : 'Sıralamaya dahil değil'} className={cn('flex min-h-9 min-w-14 shrink-0 items-center justify-center rounded-lg px-2 text-sm font-semibold tabular-nums', rank === 1 ? 'bg-success-muted text-success-foreground' : 'bg-surface-muted text-text-secondary')}>{rank && totalRanked ? `${rank}/${totalRanked}` : '–'}</span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="truncate font-medium text-text-primary">{person.name}</span>{person.isCurrentUser ? <span className="rounded-md bg-info-muted px-2 py-0.5 text-xs font-medium text-info-foreground">Siz</span> : null}{person.role === 'admin' ? <span className="rounded-md bg-surface-muted px-2 py-0.5 text-xs font-medium text-text-secondary">Temsilci</span> : null}{!person.isActive ? <span className="rounded-md bg-destructive-muted px-2 py-0.5 text-xs font-medium text-destructive-foreground">Pasif</span> : null}</span>{rank === 1 ? <span className="mt-1 flex items-center gap-1 text-xs text-success-foreground"><Target aria-hidden="true" className="size-3" />Yeni görev için ilk öncelik</span> : person.isCurrentUser && rank && totalRanked ? <span className="mt-1 block text-xs text-text-secondary">{totalRanked} kişi içinde {rank}. görevlendirme önceliği</span> : null}</span><span className="shrink-0 text-right"><span className="text-xl font-semibold tabular-nums">{person.totalPoints}</span><span className="ml-1 text-xs text-text-secondary">puan</span></span>{canViewDetails ? <ChevronDown aria-hidden="true" className={cn('size-4 shrink-0 text-text-secondary transition-transform', expanded && 'rotate-180')} /> : null}</>
  return <li className={cn(person.isCurrentUser && 'bg-info-muted/40', !person.isActive && 'bg-surface-muted/45')}>
