...
- 2026-07-13 (aynı oturum): Özellik prod'a deploy edildi (`/api/sync-workbook` canlı; prod önizleme 787 görev / 2468 puan doğrulandı). Ardından yetkili geri bildirimiyle import ekranındaki eski düz importer kartı UI'dan tamamen kaldırıldı; senkron tek görünür akış oldu. Senkron önizlemesine "sistemde hesabı olmayan kişi sayfası" (unmatchedPersonSheets) uyarısı eklendi. Commit `3546c8f`.
- 2026-07-13 (aynı oturum): "Çalışmayan koda gerek yok" talebiyle kullanılmayan eski importer tamamen silindi: `/api/import-excel/route.ts`, `import-service.ts`, `parser.ts`, `schemas.ts` ve iki birim testi kaldırıldı; `admin-data-routes.test.ts` cerrahi düzenlendi (reset-period/categories/export-excel testleri korundu). Prisma `ImportBatch`/`ImportBatchRow`/`ImportLog` modelleri, reset-period audit'i ve period-service kullandığı için korundu (silmek ayrı migration). CLAUDE.md/AGENTS.md ve SISTEM_DOKUMANTASYONU.md güncellendi. tsc temiz, tests 165 ✓, temiz `next build` ✓ (43 rota), docs:check 0.
